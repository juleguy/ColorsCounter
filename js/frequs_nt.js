...
if (location.search == "?l=fr") { //Si français
    document.title = "Proportions et relations color&eacute;es d'une image";
    ...
    document.getElementById("title").innerHTML = "Proportions et relations color&eacute;es d'une image";
    ...
    document.getElementById("loadalocalimage").innerHTML = "Chargez une image locale : ";
...
function compare(a, b) {
  if ((a.couleur.y + 2*a.couleur.u + 2*a.couleur.v)/5 < (b.couleur.y + 2*b.couleur.u + 2*b.couleur.v)/5)
     return -1;
  if ((a.couleur.y + 2*a.couleur.u + 2*a.couleur.v)/5 > (b.couleur.y + 2*b.couleur.u + 2*b.couleur.v)/5)
     return 1;
...
var drawCC2 = function (type, bins) {
    ...
    var vh = document.getElementById("vhist2");
    if (vh.hasChildNodes())
    {
        while (vh.childNodes.length >= 1)
        {
    ...
    var l = Math.min(bs2.length, (nbpc + 20));
    for (var i = nbpc; i < bs2.length; i++) {
        if (bs2[i].couleur == undefined) {continue;}
        switch (cs) {
            case "YUV" :
                var ce = yuv2rgb(bs2[i].couleur);
                break;
            case "HSL" :
                var ce = ahsv2rgb(bs2[i].couleur);
                break;
            case "CIELAB" :
                var ce = cielab2rgb(bs2[i].couleur);
                break;
            case "CIELUV" :
                var ce = cieluv2rgb(bs2[i].couleur);
                break;
        }
        var ci = document.createElement('div');
        var cc = "#" + ((1 << 24) + (ce.r << 16) + (ce.g << 8) + ce.b).toString(16).slice(1);
        ...
        var cit = "["+ce.r+":"+ce.g+":"+ce.b+"]";
        //if (i < l) {
            ci.innerHTML = "&nbsp;" + cit + " : "+bs2[i].compte;
            vh.appendChild(ci);
        //}
        blob = blob + cs + " " + JSON.stringify(bs2[i].couleur) + "; RVB : " + cit + "; nbpixels : "+bs2[i].compte + "; hex :" + cc + "\n";
        nbCouleurs++;
    }
    ...
    document.getElementById("nbCouleurs").innerHTML= "Number of colors : "+nbCouleurs;
